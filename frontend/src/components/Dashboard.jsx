...
// Inside the handleSearch function
  ...
  return (
    <div ref={dashboardRef} className="flex h-screen">
      ...
      <div className="bg-red-950 w-1/5 p-4">
          ...
          <div>
            ...
            <div className="text-orange-100 mb-4">
              ...
              <div className="flex items-center justify-start mt-auto cursor-pointer text-orange-100 hover:text-orange-200">
                ...
                {showNotification && (
                  <div
                    className={`absolute top-10 text-orange-100 left-60 items-center mt-0 p-10 transition-transform duration-600 ease-linear shadow-2xl shadow-red-950 ${
            ...
            {userRole === "DySP" && (
              <div className="mt-2">
                <select className="text-gray-900 border border-gray-300 rounded px-3 py-1 w-full bg-orange-100">
            ...
            <div className="mt-4 flex items-center space-x-4">
              ...
              <FontAwesomeIcon
                ...
                className="px-3 py-2 text-2xl bg-red-950 font-bold  text-orange-100 hover:text-orange-200 rounded-r cursor-pointer"
              />
            </div>
            ...
            {searchResult && (
              <div>
                <h2>FIR Details</h2>
                <p>FIR Number: {searchResult.firNo}</p>
                <p>District: {searchResult.districtName}</p>
                <p>Unit Name: {searchResult.unitName}</p>
                <p>FIR Date: {searchResult.firDate}</p>
                <p>FIR Stage: {searchResult.firStage}</p>
                <p>Crime Group Name: {searchResult.crimeGroupName}</p>
      ...
      <div className="flex-1 p-2 flex flex-col bg-orange-100 text-red-800 ">
        <div className="flex mb-4">
          <div className="flex flex-row gap-1 justify-between items-end w-full">
            ...
            {boxData.map((box, index) => (
              <div
                key={index}
                className="bg-red-950 px-2 py-1 mb-4 pr-2 rounded-md mr-2 ml-4 w-32 h-1/2 font-bold shadow-xl shadow-red-900 text-orange-100"
              >
                {/* Render the icon */}
                <FontAwesomeIcon
                  icon={box.icon}
        ...
        <div className="flex mt-4">
          ...
          <div className="bg-orange-100 flex-1 mr-16 p-0 rounded-md shadow-red-900 shadow-xl ml-4">
            <table className="table-auto w-full m-0 p-0">
              <thead className="bg-red-950 text-red-100">
                <tr>
                  <th className="p-3">SN Number</th>
                  <th className="p-3">FIR ID</th>
                  <th className="p-3">FIR Stage</th>
                  <th className="p-3">FIR Date</th>
                  <th className="p-3">FIR CrimeType</th>
                </tr>
              </thead>
              <tbody>
                {recentList.map((row, index) => (
                  ...
                  >
                    <td className="text-center p-1">{row.snNumber}</td>
                    <td className="text-center">{row.firId}</td>
                    <td className="text-center">{row.firStage}</td>
                    <td className="text-center">{row.firDate}</td>
                    <td className="text-center">{row.firCrimeType}</td>
                  </tr>
                ))}
              </tbody>
            </table>
